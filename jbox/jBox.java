...
/*
	...
	Return: javascript eval:
		jBox_distance(name, s, t, n)
			s = current segment length
			t = running total length
			n = number of vertices (for debugging)

	Compiler issues:
		Sun JDK 1.2 & 1.3 work ok,
...
public class jBox extends Applet implements MouseListener, MouseMotionListener {
    ...
    double x1=-1, y1=-1, x2=-1, y2=-1, s=0, t=0;
	...
	if(verbose)
	    window.eval("javascript:jBox_mouse_moved('" + this.name + "'," + event.getX() + "," + event.getY() + ");");
	if(line) {
	    x2 = event.getX();
	    y2 = event.getY();
	    repaint();
	...
	if (line) {
	    ...
	    if (pl.npoints > 0) {	// don't rubberband w/o starting point
		offScreenGraphics.drawLine((int)x1, (int)y1, (int)x2, (int)y2);
		s = Math.sqrt(Math.pow(y2 - pl.ypoints[pl.npoints-1],2) + Math.pow(x2 - pl.xpoints[pl.npoints-1],2));
	    }
	    
	    window.eval("javascript:jBox_distance('" + name + "'," + s + "," + t +"," + pl.npoints + ");");
